...
class AuthController extends BaseController
{
    ...
    public function loginHandlerTg(TelegramLoginAuth $telegramLoginAuth, Request $request)
    {
        //var_dump($request->all());exit();
        if ($user = $telegramLoginAuth->validate($request)) {
            $credentials = $request->all();
            $userinfo = User::query()->where('telegram_id', $credentials['id'])->first();
            if (! $userinfo){
                $usertg = new User();
                $usertg->telegram_id = $credentials['id'];
                $usertg->telegram_nick = $credentials['first_name']." ".$credentials['last_name'];
                $usertg->telegram_username = $credentials['username'];
                $usertg->password = bcrypt("123456");
                $usertg->last_ip = $request->ip();
                $usertg->last_login = now()->toDateTimeString();
                $usertg->register_at = now()->toDateTimeString();
                if (isset($credentials['invite_code'])){
                    $usertg->pid = User::query()->where('invite_code', $credentials['invite_code'])->value('id');
                }
                $usertg->invite_code = Str::random(8);
                $usertg->save();
            }else{
                ...
                $userinfo->last_login = now()->toDateTimeString();
                $userinfo->save();
            }
            $data = ["telegram_id"=>$credentials['id'],"telegram_username"=>$credentials['username'],"password"=>"123456"];
        ...
        //return $this->err('账号密码错误');
        ///www/server/php/74/bin/php artisan vendor:publish --provider="Azate\LaravelTelegramLoginAuth\Providers\LaravelServiceProvider" --tag=config
    }
