 ...
 public function validatorCreateOrder(Request $request): void
{
     ...
     * @link      http://utf8.hk/
     */
    public function validatorGoodsTg($request): Goods
    {
        // 获得商品详情
        $goods = $this->goodsService->detail($request['gid']);
        // 商品状态验证
        $this->goodsService->validatorGoodsStatus($goods);
        // 如果有限购
        if ($goods->buy_limit_num > 0 && $request['by_amount'] > $goods->buy_limit_num) {
            throw new RuleValidationException(__('dujiaoka.prompt.purchase_limit_exceeded'));
        }

        // 如果有最低购买限制
        if ($goods->min_buy_num > 0 && $request['by_amount'] < $goods->min_buy_num) {
            $actualAmount = $request['by_amount'];
            $minRequiredAmount = $goods->min_buy_num;
            throw new RuleValidationException(__("当前购买数量：:actualAmount，最低购买数量：:minRequiredAmount", ['actualAmount' => $actualAmount, 'minRequiredAmount' => $minRequiredAmount]));
        }

        // 库存不足
        if ($request['by_amount'] > $goods->in_stock) {
            throw new RuleValidationException(__('dujiaoka.prompt.inventory_shortage'));
        }
        // 预选，且预选卡密不存在
        if(isset($request['carmi_id'])){
            if(!$this->goodsService->checkCarmiBelong($goods['id'], $request['carmi_id'])){
                throw new RuleValidationException(__('dujiaoka.prompt.preselect_unable'));
            }
        }
        return $goods;
     ...
     * @link      http://zhangyiqiu.net/
     */
    public function validatorLoopCarmisTg($request)
    {
        $carmis = Carmis::query()
            ...
            ->where('is_loop', true)
            ->count();
        if($carmis > 0 && $request['by_amount'] > 1){
            throw new RuleValidationException(__('dujiaoka.prompt.loop_carmis_limit'));
        }
        return $carmis;
    }
     ...
     * @link      http://utf8.hk/
     */
    public function validatorCoupon($request):? Coupon
    {
        // 如果提交了优惠码
        if (isset($request['coupon_code'])) {
            // 查询优惠码是否存在
            $coupon = $this->couponService->withHasGoods($request['coupon_code'], $request['gid']);
            // 此商品没有这个优惠码
            if (empty($coupon)) {
