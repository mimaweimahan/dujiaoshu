...
class GoodsService
{
  ...
  public function withGroup(): ?array
    {
        // 你可以在这里添加逻辑来决定使用哪种排序方式。
        // 例如，使用dujiaoka_config('jg')的返回值来决定排序规则。
        $orderBy = dujiaoka_config_get('jg') == 'price' ? 'actual_price' : 'ord';
        $orderDirection = dujiaoka_config_get('jg') == 'price' ? 'ASC' : 'DESC';
        ...
        $goods = GoodsGroup::query()
            ->with(['goods' => function($query) use ($orderBy, $orderDirection) {
                ...
                ->orderBy($orderBy, $orderDirection); // 根据配置决定排序方式
            }])
            ->where('is_open', GoodsGroup::STATUS_OPEN)
            ->orderBy('ord', $orderDirection) // 分类的排序也可以根据配置进行调整
            ->get();
     ...
     * @link      http://utf8.hk/
     */
    public function withGroupTelegram(): ?array
    {
        // 你可以在这里添加逻辑来决定使用哪种排序方式。
        // 例如，使用dujiaoka_config('jg')的返回值来决定排序规则。
        $orderBy = dujiaoka_config_get('jg') == 'price' ? 'actual_price' : 'ord';
     ...
     * @link      http://utf8.hk/
     */
    public function goodsClass(int $id)
    {
        $goods = Goods::query()
        ...
        return $goods->toArray() ? $goods->toArray() : null;
    }
