    ...
    public function form()
    {
        ...
        $this->tab(admin_trans('访问/登录充值限制'), function () {
          ...
          $this->tab(admin_trans('商品推送配置'), function() {
                ...
                $this ->text('telegram_bot_username', admin_trans('TelegramBotUsername'));
                $this ->text('telegram_api_proxy', admin_trans('Telegram API代理'));
                $this ->text('telegram_chat_id', admin_trans('商品推送群或频道ID'));
                $this ->text('telegram_chat_id_notify', admin_trans('telegram订单通知用户或群id'));
        ...
        $this->tab(admin_trans('其他api设置'), function () {
            $this->text('energy_api_token', admin_trans('能量租用apikey'));
            $this->text('energy_api_secret', admin_trans('能量租用apisecret'));
            $this->text('energy_buy_config', admin_trans('能量租用购买配置json'));
            $this->text('energy_address', admin_trans('能量收款地址'));
            $this->text('tronscan_api', admin_trans('tronscan密钥'));
            $this->text('subscribe_api', admin_trans('开会员api地址'));
            $this->text('subscribe_buy_config', admin_trans('开会员配置'));
            $this->text('fragment_hash', admin_trans('fragment的hash'));
            $this->text('fragment_cookie', admin_trans('fragment的cookie'));
            $this->select('chang_huiyuan', admin_trans('system-setting.fields.chang_huiyuan'))
                ->options(config('dujiaoka.huiyuan'))
                ->required();
            $this->text('search_keyword', admin_trans('群组搜索关键词'));
        });

        $this->tab(admin_trans('其他配置'), function () {
            $this->image('pay_image', admin_trans('支付二维码图片'));
            $this->text('pay_image_api', admin_trans('支付二维码api设置'));
            $this->text('huilv', admin_trans('Epusdt支付汇率'));
            $this->number('mini_deposit_amount', admin_trans('最低充值金额'));
            $this->number('max_deposit_amount', admin_trans('最大充值金额'));
            $this->switch('submit_search', admin_trans('是否开启创建商品提交搜索引擎'))->default(BaseModel::STATUS_OPEN);
            $this->text('recharge_text', admin_trans('充值展示的金额符号'));
            $this->text('license_code', admin_trans('授权码'))->required();
        });
