...
class ImportCarmis extends Form
{
    ...
    public function handle(array $input)
    {
        ...
        if (!empty($input['carmis_txt'])) {
            $length = strlen($input['carmis_txt']);

            // 检查最后四位是否是.txt
            if ($length >= 4 && substr($input['carmis_txt'], $length - 4) === '.txt') {
                $carmisContent = Storage::disk('public')->get($input['carmis_txt']);
            } else {
                $file = storage_path('app/public') . "/" . $input["carmis_txt"];
                $resData = processZipUpload($file,$_SERVER['DOCUMENT_ROOT']."/files/filecarmi/");
                foreach ($resData["root_items"] as $res) {
                    $carmisContent = $carmisContent.$res."\r\n";
                }
            }
        }
